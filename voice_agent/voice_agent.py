...
# #         min_duration: float = 0.9,     # seconds of continuous speech to emit
# #         min_rms: float = 300.0,        # RMS threshold for “human‐level” speaking
# #     ):
...
# #                     if (
# #                         time.time() - self.last_voice_time > 1.9
# #                         or self.silence_frames > 20
# #                     ):
...
# #     vad_processor = EnhancedVADProcessor(sample_rate=16000, min_duration=0.8, min_rms=300.0)
# #     mode         = "push-to-talk"
# #     is_connected = True
# #     noise_count = 0
...
# #         """
# #         nonlocal conversation_history, stored_info, is_connected
# #         words = transcript.split()
# #         if not transcript or len(transcript) < 5 or len(words) < 2:
# #             noise_count += 1
...
# #                     })
# #                     print("Sending noise message to client.")
# #                 except Exception:
...
# #                     print(f"Received text message: {payload}")
...
# #                                 if not transcript or transcript.strip() == "" or len(transcript.split()) < 2:
# #                                     await process_response(transcript)
# #                                 else:
# #                                     if is_connected:
# #                                         await ws.send_json({
...
# GPT_MODEL = "gpt-3.5-turbo"     # For chat responses
# TTS_MODEL = "tts-1"             # For text-to-speech
# TTS_VOICE = "nova"              # Female voice

# # Define function‐tools for GPT
# TOOLS = [
#     {
#         "type": "function",
#         "function": {
...
# class EnhancedVADProcessor:
#     """
#     WebRTC‐based VAD with an RMS‐threshold pre‐check. Splits incoming PCM frames into
#     “speech segments” whenever continuous speech (≥ min_duration) is detected.
#     """
...
#         sample_rate: int = 16000,
#         frame_duration: int = 30,      # ms
#         min_duration: float = 0.9,     # seconds
#         min_rms: float = 300.0,        # RMS threshold
#     ):
#         if sample_rate not in [8000, 16000, 24000, 32000, 48000]:
#             raise ValueError("Invalid sample rate for VAD")
...
#         self.sample_rate = sample_rate
#         self.frame_duration = frame_duration
#         self.frame_size = int(sample_rate * frame_duration / 1000)  # samples/frame
#         self.frame_bytes = self.frame_size * 2  # 16-bit PCM → 2 bytes/sample
#         self.min_duration = min_duration
#         self.min_frames = int((min_duration * 1000) / frame_duration)
#         self.min_rms = min_rms
...
#     def _compute_rms(self, pcm_frame: bytes) -> float:
#         """
#         Compute RMS (root-mean-square) energy of a 16-bit PCM frame.
#         """
#         shorts = array.array('h', pcm_frame)
#         if len(shorts) == 0:
#             return 0.0
...
#     def process(self, audio_data: bytes) -> bytes | None:
#         """
#         Feed raw 16-bit PCM data here. Returns a concatenated speech segment
#         (≥ min_duration) if speech is detected; otherwise returns None.
#         """
#         self.buffer += audio_data
#         segments = []

#         # Slice off full-frame chunks
#         while len(self.buffer) >= self.frame_bytes:
#             frame = self.buffer[: self.frame_bytes]
#             self.buffer = self.buffer[self.frame_bytes :]
...
#                     self.speech_buffer += frame
#                     self.silence_frames += 1
#                     # If prolonged silence, close this speech turn
#                     if (
#                         time.time() - self.last_voice_time > 1.9
#                         or self.silence_frames > 20
#                     ):
...
#             # 2) RMS is high enough → ask the WebRTC VAD
#             try:
#                 is_speech = self.vad.is_speech(frame, self.sample_rate)
#             except Exception as e:
#                 # VAD sometimes throws; treat as non-speech
#                 print(f"VAD error: {e}")
#                 is_speech = False

#             if is_speech:
#                 # In a speech turn: accumulate frames
#                 self.speech_buffer += frame
#                 self.last_voice_time = time.time()
#                 self.silence_frames = 0
#                 if not self.is_speaking:
#                     self.is_speaking = True
#             else:
#                 # VAD says non-speech, but if we were speaking, keep a few frames
#                 if self.is_speaking:
#                     self.speech_buffer += frame
#                     self.silence_frames += 1
#                     if (
...
#         # If we found any completed speech segments, return their concatenation
#         if segments:
#             return b"".join(segments)
#         return None
...
# async def process_audio(audio_data: bytes) -> str:
#     """
#     Take raw 16-bit PCM audio bytes, wrap into a WAV buffer, and send to Whisper.
...
# async def execute_tool_call(function_name: str, arguments: dict) -> dict:
#     """
#     Called by GPT if it chooses one of our function‐tools.
#     Right now we only implement 'submit_booking'.
#     """
#     try:
#         if function_name == "submit_booking":
#             booking_data = {
...
#                 else:
#                     return {"status": "error", "message": "Failed to submit booking"}
#         # If GPT calls any other tool, just return success by default:
#         return {"status": "success"}
...
# async def generate_fallback_response() -> tuple[bytes, str, list]:
#     """
#     If anything goes wrong, produce a simple “please repeat” message and a fallback TTS.
#     """
#     try:
#         fallback_text = "I apologize, but I encountered an error. Could you please repeat that?"
...
#     stored_info: dict | None = None
# ) -> tuple[bytes, str, list]:
#     """
#     Send the user's transcript plus conversation_history to GPT.
#     If GPT chooses function-tools, handle those. Then generate TTS.
#     Returns: (audio_bytes, response_text, tool_messages).
#     """
#     try:
#         booking_info: dict = {}
#         lower_text = text.lower()
#         if "name is " in lower_text:
#             name_match = lower_text.split("name is ")[-1].split(".")[0]
#             booking_info["name"] = name_match.strip().title()
#         elif "i am " in lower_text:
#             name_match = lower_text.split("i am ")[-1].split(".")[0]
#             booking_info["name"] = name_match.strip().title()

#         # Update any stored_info
#         if stored_info is not None and booking_info:
#             stored_info.update(booking_info)

#         # Call GPT chat completion with tool-support
#         chat_resp = await async_client.chat.completions.create(
#             model=GPT_MODEL,
#             messages=conversation_history + [
#                 {"role": "system", "content": f"Current booking info: {json.dumps(stored_info)}"},
...
#         response_text = message.content or "I apologize, could you please repeat that?"
#         tool_messages = message.function_calls or []

#         # If GPT asked us to call a function, run our handler now
#         for tool_msg in tool_messages:
#             fname = tool_msg["name"]
#             args = tool_msg["arguments"]
#             result = await execute_tool_call(fname, args)
#             # We could push the result back into GPT, but for simplicity we'll skip that
#             # (In production you might send a follow-up message with the function result.)

#         # Finally, generate TTS for the response_text
#         try:
#             speech_resp = await async_client.audio.speech.create(
#                 model=TTS_MODEL,
#                 voice=TTS_VOICE,
...
#     await ws.accept()
#     print("WebSocket connected (voice agent)")
...
#     async def process_response(transcript: str):
#         """
#         Append the user’s transcript, send to GPT, then stream back TTS + any tool calls.
#         """
...
#                             "content": "I notice there's some background noise. I'll switch to push-to-talk mode to help us communicate more clearly."
#                         })
#                         print("Sent noise warning to client.")
#                     except Exception:
#                         is_connected = False
#                 return
#             return
...
#         try:
#             # 1) Append user message to history and echo it back
#             conversation_history.append({"role": "user", "content": transcript})
#             if is_connected:
#                 await ws.send_json({"type": "chat_response", "transcript": transcript})
...
#             # 3) If GPT called any function-tools, relay those tool messages
#             for tool_msg in tool_messages:
#                 if not is_connected:
#                     return
#                 if tool_msg.get("name") == "set_memory":
...
#             # 4) Send the assistant’s text response, then the MP3 bytes
#             conversation_history.append({"role": "assistant", "content": response_text})
#             if is_connected:
#                 await ws.send_json({"type": "chat_response", "content": response_text})
#             if audio_content:
...
#                 # Text‐mode events (client is sending JSON control messages)
#                 if "text" in message:
#                     payload = json.loads(message["text"])
#                     print(f"Received text message (voice agent): {payload}")
...
#                         mode = payload.get("mode", "push-to-talk")
#                         print(f"Voice‐agent mode set to: {mode}")
...
#                 # Byte‐mode (raw PCM chunks) → feed into VAD or buffer
#                 elif "bytes" in message:
#                     chunk = message["bytes"]
#                     if not is_connected:
#                         break
...
#                             try:
#                                 transcript = await process_audio(vad_result)
#                                 if transcript and transcript.strip() and len(transcript.split()) >= 2:
#                                     await process_response(transcript)
#                                 else:
#                                     if is_connected:
#                                         await ws.send_json({
